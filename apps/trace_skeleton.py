...
# -------------------------------------------------------------------------------        #추가(
    # im0 = cv2.imread(path)
    im0 = np.fromfile(path, np.uint8)              ### 이미지 데이터에 한글 경로가 있을 경우, np.fromfile함수를 통해 바이너리 데이터를 넘파이 행렬로
    im0 = cv2.imdecode(im0, cv2.IMREAD_COLOR)       ### cv2.imdecode를 통해 복호화 과정을 거쳐서 opencv에서 이용할 수 있는 형태로 변환

    from apps.fix import fix                         # fix.py에서 전처리 클래스 호출
    ...
    im0 = fix(im0).threshold()

    # 노이즈 제거 및 선명화 작업
    im0 = fix(im0).Noise()
# -------------------------------------------------------------------------------      # 추가)
    im0 = cv2.cvtColor(im0, cv2.COLOR_GRAY2BGR)                                        # 추가
...
# -------------------------------------------------------------------------------      # 추가(
    ...
    a  = cv2.cvtColor(a, cv2.COLOR_GRAY2BGR)                                           # 추가
# -------------------------------------------------------------------------------      # 추가)
    for l in polys:
        # c1 = (200*random.random(),200*random.random(),200*random.random()) # 랜덤 색으로 그리기임.
        c1 = (0, 0, 255)                                                      # 빨간 선으로 그리기
        c2 = (255, 255, 255)                                                # 화이트 선으로 그리기
        for i in range(0,len(l)-1):
            cv2.line(im0,(l[i][0],l[i][1]),(l[i+1][0],l[i+1][1]),c1)
            cv2.line(a,(l[i][0],l[i][1]),(l[i+1][0],l[i+1][1]),c2)          # 추가

    # cv2.imshow('',im0);cv2.waitKey(0)
    # plt.imshow(im0)
    # plt.show()
    
    return im0, a    ### Trace 첫번째 인자는 이미지에 trace_skeleton 적용, 두번째 인자는 trace_skeleton 부분만 추출