    ...
    if (image is not None) and (st.session_state.button_clicked):
        ...
        with st.spinner('Performance evaluation in progress ...'):
            ...
            for i in stqdm(location, desc='This is slow task'):
                ...
                ss = []
                for j in location:
                    print(os.path.join(os.getcwd(), 'dataset', '레지스트', i))
                    paths = (os.path.join(recon_data, os.path.splitext(j)[0], os.path.basename(image))).str.replace('\\', '/')
                    path = (os.path.join(os.getcwd(), 'dataset', '레지스트', i)).str.replace('\\', '/')
